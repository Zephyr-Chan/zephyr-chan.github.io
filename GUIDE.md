...
13. [如何管理 Resources 资源导航页面](#13-如何管理-resources-资源导航页面)
14. [如何管理 Social 社交页面](#14-如何管理-social-社交页面)
15. [如何管理访客地图 ClustrMaps](#15-如何管理访客地图-clustrmaps)
16. [如何替换头像和图片](#16-如何替换头像和图片)
17. [如何修改配色](#17-如何修改配色)
18. [导航栏与中英文切换系统](#18-导航栏与中英文切换系统)
...
这是一个**纯静态学术个人主页**，不需要服务器、不需要数据库。网站由 5 个页面组成，共享同一套样式和脚本。
...
| 算法竞技场 | `algorithms.html` | OJ 平台链接、算法模板、VP记录、竞赛经历 |
| 资源导航 | `resources.html` | 会议截止时间、学习资料、实用工具链接 |
...
3. 把以下文件和文件夹**全部**拖进去（注意要在根目录，不要嵌套）：
   - `index.html`、`cv.html`、`algorithms.html`、`resources.html`、`social.html`
   - `css/` 文件夹
   - `js/` 文件夹
   - `assets/` 文件夹
4. 点击 `Commit changes`
...
算法页面 (`algorithms.html`) 包含 OJ 平台链接、算法模板、VP 记录和竞赛经历。
...
搜索 `algo-tab`，找到算法模板区域。每个 `algo-item` 是一个模板分类，修改图标、名称和描述。

### 12.3 添加 VP 记录

搜索 `algo-empty`，找到 VP 空状态占位区域，替换为实际的 VP 记录列表。

**注意**：由于这是纯静态网站，**无法在线添加/上传内容**。所有内容都需要在 HTML 文件中手动编辑，然后重新部署到 GitHub Pages。

如果你需要在线添加内容的功能，有以下方案：
- **方案 A**：使用 GitHub Issues/ Discussions 作为内容管理后台，通过 GitHub API 读取内容显示在页面上
- **方案 B**：使用 Google Sheets 作为数据库，通过 Google Sheets API 读取内容
- **方案 C**：使用 CMS 服务（如 Netlify CMS、Decap CMS）配置 Git-based 内容管理
...
推荐参考 [silencer76.com](https://silencer76.com) 的算法板块设计。

---

## 13. 如何管理 Resources 资源导航页面

资源导航页面 (`resources.html`) 是一个精选链接集合，包含会议截止时间、学习资料和实用工具链接。

### 13.1 修改会议截止时间链接

搜索 `resource-grid`，找到会议卡片区域。每个 `resource-card` 是一个链接，修改 `href`、图标颜色和描述即可。

### 13.2 添加学习资料

搜索 `resources_learning`，找到学习资料区域。把占位卡片改为实际链接：

```html
<a href="https://你的链接" target="_blank" rel="noopener" class="resource-card">
  <div class="resource-card-icon" style="background: #颜色;">
    <i class="fas fa-icon-name"></i>
  </div>
  <div class="resource-card-info">
    <div class="resource-card-title">资料名称</div>
    <div class="resource-card-desc">简短描述</div>
  </div>
  <div class="resource-card-arrow"><i class="fas fa-chevron-right"></i></div>
</a>
```

删除 `Coming Soon` 徽章（`coming-soon-badge` span）即可。

### 13.3 添加实用链接

搜索 `link-list`，在列表中复制一个 `link-item` 并修改。

### 13.4 推荐的会议截止时间查询工具

| 工具 | 链接 | 说明 |
|------|------|------|
| CCF Deadline | https://ccfddl.github.io/ | CCF推荐会议截止日期 |
| AI Conference Deadlines | https://aideadlin.es/ | AI/ML顶会截止日期 |
| Conference Tracker | https://conferencetracker.com/ | 全球学术会议追踪 |
| WikiCFP | https://wikicfp.com/ | CFP征集信息 |
...
```
About | Education | Research | News | Publications | Projects | Honors | Skills | Gallery | Contact | CV | Algorithms | Resources | Social
```
...
搜索 `navbar-links`，添加/删除 `<a>` 标签。**所有 5 个页面的导航栏都要同步修改。**
